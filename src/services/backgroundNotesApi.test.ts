...
describe("local attachments", () => {
  it("reads a nonempty regular file and rejects unsafe shapes and sizes", () => {
    const directory = mkdtempSync(join(tmpdir(), "background-attachment-"));
    try {
      ...
      expect(() => localAttachment(large)).toThrow(/64 MiB/);
    } finally {
